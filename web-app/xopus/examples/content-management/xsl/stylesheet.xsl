...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="article|section">
        <div class="contents">
            <xsl:choose>
                <xsl:when
                        test="$hideSectionContent = 'false' or (generate-id(.) = generate-id($currentNode) or .//*[generate-id(.) = generate-id($currentNode)])">
                    ...
                    <a href="#{generate-id(.)}">
                        <xsl:if test="$hideSectionContent = 'true'">
                            <xsl:attribute name="onclick">
                                Editor.Canvas.getActiveCanvas().setViewParam("currentNode",node);
                            </xsl:attribute>
                        </xsl:if>
    ...
    <xsl:template match="section" mode="toc">
        <li>
            <a href="#{generate-id(.)}">
                <xsl:if test="$hideSectionContent = 'true'">
                    <xsl:attribute name="onclick">Editor.Canvas.getActiveCanvas().setViewParam("currentNode",node);
                    </xsl:attribute>
                </xsl:if>
    ...
    <xsl:template match="example">
        ...
        <xsl:if test="$xopus = 'true' and @show-execute-button = 'true'">
            <script>function <xsl:value-of select="generate-id(.)"/>(node) {<xsl:value-of select="."/>}
            </script>
