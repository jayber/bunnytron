...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="section[@view='tabs']/section/h" mode="tab-item">
        <div>
            <xsl:attribute name="class">tab-item
                <xsl:if test="../processing-instruction('expanded')">chosen-tab-item</xsl:if>
            </xsl:attribute>
            <div class="corner corner-left-top"></div>
            <xsl:if test=".. = $internal-nav/*">
                <div class="chosen-tab-item-left"></div>
                <div class="chosen-tab-item-top"></div>
            </xsl:if>
            <h4>
                <xsl:apply-templates/>
            </h4>
            <div class="corner corner-right-top"></div>
            <xsl:if test="../processing-instruction('expanded')">
                <div class="chosen-tab-item-right"></div>
            </xsl:if>
        </div>
    </xsl:template>
        ...
        <td valign="top">
            ...
            <div>
                <xsl:if test="@division">
                    <xsl:attribute name="class">divopen
                        <xsl:if test="count(ancestor::person) = 0">topchart</xsl:if>
                    </xsl:attribute>
                    <div class="divname">
    ...
    <xsl:template match="entry">
        ...
        <xsl:variable name="span"
                      select="count(../../../colspec[following-sibling::colspec[@colname = current()/@nameend]]) - count(../../../colspec[following-sibling::colspec[@colname = current()/@namest]]) + 1"/>
        <td>
    ...
    <xsl:template match="note">
        <span class="note-nr">[<xsl:number count="note" level="any" format="1"/>]
        </span>
    </xsl:template>
    ...
    <xsl:template match="@text-align">
        <xsl:attribute name="style">text-align:<xsl:value-of select="."/>
        </xsl:attribute>
    </xsl:template>
    ...
    <xsl:template match="@align">
        ...
        <xsl:attribute name="class">align-<xsl:value-of select="."/>
        </xsl:attribute>
    </xsl:template>
    ...
    <xsl:template match="video[contains(@src, 'vimeo.com')]" mode="embedded-video">
        ...
        <xsl:param name="uri"
                   select="concat('http://vimeo.com/moogaloop.swf?clip_id=', $id, '&amp;server=vimeo.com&amp;show_title=1&amp;show_byline=0&amp;show_portrait=0&amp;color=ffffff&amp;fullscreen=1')"/>
