...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="section/title">
        <h2><xsl:number count="article|section" from="/article" level="multiple" format="1"/>.
            <xsl:apply-templates select="." mode="tagsinline"/>
        </h2>
    </xsl:template>
    ...
    <xsl:template match="image">
        ...
        <img src="{@src}">
            ...
            <xsl:if test="@halign and @halign!='none'">
                ...
                <xsl:attribute name="class">align-<xsl:value-of select="@halign"/>
                </xsl:attribute>
            </xsl:if>
        </img>
        ...
        <div align="center">
            <table border="0" width="{@width}%">
                ...
                <xsl:attribute name="class">
                    <xsl:choose>
                        <xsl:when test="@color">color_<xsl:value-of select="@color"/>
                        </xsl:when>
                        <xsl:otherwise>color_grey</xsl:otherwise>
                    </xsl:choose>
                    <xsl:choose>
                        <xsl:when test="@border">border_<xsl:value-of select="@border"/>
                        </xsl:when>
    ...
    <xsl:template match="tbody">
        <tbody>
            <xsl:attribute name="class">
                <xsl:text>rowcoloring_</xsl:text>
                <xsl:choose>
                    <xsl:when test="../@alternaterowcolor='true' or ../@alternaterowcolor='1'">alternate</xsl:when>
    ...
    <xsl:template match="*" mode="starttag">
        <span class="starttag" unselectable="on">
            <xsl:value-of select="name()"/>
        </span>
    </xsl:template>

    <xsl:template match="*" mode="endtag">
        <span class="endtag" unselectable="on">
            <xsl:value-of select="name()"/>
        </span>
    </xsl:template>
