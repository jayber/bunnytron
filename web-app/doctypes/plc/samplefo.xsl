...
<xsl:stylesheet version="1.0"
        ...
        <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
            <fo:layout-master-set>
                ...
                <fo:page-sequence-master master-name="layout"
                    ...
                    <fo:single-page-master-reference master-name="firstA4"/>
            ...
            <fo:page-sequence master-name="layout">
                <fo:static-content flow-name="xsl-region-before">
                                        ...
                                        <xsl:choose>
                                            <xsl:when test="$type='Skills Checklist'">
                                                <fo:external-graphic content-type="content-type:image/tiff"
                                                                     src="\\CIRCLE\drive_g\Common\img\xslfo\skills.tif"/>
                                            </xsl:when>
                                            <xsl:otherwise>
                                                <!--<fo:external-graphic content-type="content-type:image/tiff" src="logo.tif"/>-->
                                                <fo:external-graphic content-type="content-type:image/tiff"
                                                                     src="\\CIRCLE\drive_g\Common\img\xslfo\logo.tif"/>
                                            </xsl:otherwise>
                                        </xsl:choose>
                                    </fo:block>
                                </fo:table-cell>
                    ...
                    <!--
                    <fo:block>
                    <fo:external-graphic content-type="content-type:image/tiff" src="\\CIRCLE\drive_g\Common\img\xslfo\logo.tif"/>
                    </fo:block>
                            ...
                            <fo:table-row>
                                <fo:table-cell>
                                    <fo:block space-before="8" text-align="left" font-size="9pt" font-weight="bold"
                                              font-family="'News Gothic MT'"><xsl:if test="$type!='Skills Checklist'">
                                        www.practicallaw.com/</xsl:if>A<xsl:value-of select="*/metadata/resourceid"/>
                                    </fo:block>
                                </fo:table-cell>
                                <fo:table-cell>
    ...
    <xsl:template match="fulltext">
        ...
        <xsl:if test="$type='Skills Checklist'">
            <fo:block>
                <fo:external-graphic content-type="content-type:image/tiff">
                    <xsl:attribute name="src">\\CIRCLE\drive_g\Common\img\xslfo\lines.tif</xsl:attribute>
                </fo:external-graphic>
            </fo:block>
            <fo:block>
                <xsl:call-template name="make-block">
            ...
            <xsl:for-each select="//graphic">
                ...
                <fo:block>
                    ...
                    <xsl:variable name="full_img">
                        <xsl:choose>
                            <xsl:when test="$usetif='yes'">\\CIRCLE\drive_g\common<xsl:value-of select="$img_relpath"/>.tif
                            </xsl:when>
                            <xsl:otherwise>\\CIRCLE\drive_g\common<xsl:value-of select="attribute::fileref"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:variable>
                    ...
                    <!--<fo:external-graphic content-type="content-type:image/tiff"><xsl:attribute name="src">\\CIRCLE\drive_g\common<xsl:value-of select="attribute::fileref"/></xsl:attribute></fo:external-graphic>-->
                </fo:block>
            </xsl:for-each>
        </xsl:if>
        ...
        <xsl:if test="//section1[attribute::layout='box']">
            <fo:block break-before="page"></fo:block>
            <fo:block>
                <xsl:call-template name="make-block">
                    <xsl:with-param name="blockstyle">SectionHead</xsl:with-param>
            ...
            <xsl:for-each select="//section1[attribute::layout='box']">
                <fo:block>
                    <fo:external-graphic content-type="content-type:image/tiff">
                        <xsl:attribute name="src">\\CIRCLE\drive_g\Common\img\xslfo\line.tif</xsl:attribute>
                    </fo:external-graphic>
                </fo:block>
                    ...
                    <fo:table-row>
                        <fo:table-cell>
                            ...
                            <fo:block>
                                ...
                                The fulltext is available at http://www.practicallaw.com/A<xsl:value-of
                                    select="preceding-sibling::metadata/resourceid"/>
                            </fo:block>
                            <fo:block>
                                <xsl:call-template name="make-block">
                            ...
                            <fo:block>
                                ...
                                &#183; Article ID: A<xsl:value-of select="preceding-sibling::metadata/resourceid"/>
                            </fo:block>
                            <xsl:if test="preceding-sibling::metadata/type">
                                ...
                                <fo:block>
                                    ...
                                    &#183; http://www.practicallaw.com/T<xsl:value-of
                                        select="preceding-sibling::metadata/type/plcxlink/*[1]/attribute::*[1]"/>
                                </fo:block>
                            </xsl:if>
        ...
        <fo:block>
            ...
            &#169; Legal &amp; Commercial Publishing Limited 1990 - 2002. http://www.practicallaw.com.
        </fo:block>
        <xsl:choose>
            <xsl:when test="$type='Skills Checklist'">
                <fo:block>
...
-->
    ...
    <xsl:template match="para[parent::listitem/parent::itemizedlist]">
        <fo:block>
            <xsl:call-template name="make-block">
                <xsl:with-param name="blockstyle">Body</xsl:with-param>
            </xsl:call-template>
    ...
    <xsl:template match="title[parent::metadata]">
        ...
        <fo:block>
            ...
            Reference:<xsl:if test="$type!='Skills Checklist'">www.practicallaw.com/</xsl:if>A<xsl:value-of
                select="preceding-sibling::resourceid"/>
        </fo:block>
    </xsl:template>

    <xsl:template match="author[parent::metadata]">
    </xsl:template>

    <xsl:template match="contributor[parent::metadata]">
    </xsl:template>

    <xsl:template match="metadata/abstract">
    </xsl:template>

    <xsl:template match="metadata/datecreated">
    </xsl:template>

    <xsl:template match="metadata/datevalid">
    </xsl:template>

    <xsl:template match="metadata/resourceid">
    </xsl:template>

    <xsl:template match="metadata/identifier">
    </xsl:template>

    <xsl:template match="metadata/jurisdiction">
    </xsl:template>

    <xsl:template match="metadata/subject">
    </xsl:template>

    <xsl:template match="metadata/section">
    </xsl:template>

    <xsl:template match="metadata/relation">
    </xsl:template>

    <xsl:template match="metadata/type">
    </xsl:template>
    ...
    <xsl:template match="graphic">
        <fo:block>
            ...
            <fo:inline font-style="italic" font-weight="bold">
                (See Figure
                <xsl:number format="1" value="count(preceding::graphic)+1"/>
                at end of document.)
            </fo:inline>
        </fo:block>
    </xsl:template>
            ...
            <fo:table-body>
                <fo:table-row>
                    ...
                    <fo:table-cell>
                        <fo:block>
                            <fo:external-graphic content-type="content-type:image/tiff"
                                                 src="\\CIRCLE\drive_g\Common\img\xslfo\box.tif"/>
                        </fo:block>
                    </fo:table-cell>
                </fo:table-row>
            </fo:table-body>
    ...
    <xsl:template match="entry">
        <fo:table-cell>
            ...
            <fo:block font-family="'Sabon MT', 'Arial', sans-serif" border-left-color="black">
                <xsl:choose>
    ...
    <xsl:template match="plcxlink">
        <xsl:choose>
            <xsl:when test="contains (*[1]/attribute::*[1], '#')">
                ...
                A<xsl:value-of select="substring-before (*[1]/attribute::*[1],'#')"/>)
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="*[1]"/>
                (<xsl:if test="$type!='Skills Checklist'">www.practicallaw.com/</xsl:if>A<xsl:value-of
                    select="*[1]/attribute::*[1]"/>)
            </xsl:otherwise>
        </xsl:choose>
        <xsl:apply-templates/>
