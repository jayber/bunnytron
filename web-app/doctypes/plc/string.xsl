...
<xsl:stylesheet version="1.0"
    ...
    <doc:reference xmlns="">
        <referenceinfo>
            <releaseinfo role="meta">
                $Id: string.xsl,v 1.1.1.1 2006/08/17 08:46:01 beechamc Exp $
            </releaseinfo>
            <author>
    ...
    <doc:template name="str:capitalise" xmlns="">
        ...
        <refdescription>
            <para>Converts first character of string to an uppercase letter. All remaining characters are converted to
                lowercase.
            </para>
        </refdescription>
                ...
                <varlistentry>
                    ...
                    <listitem>
                        <para>Boolean controlling whether the string becomes an
                            UpperCamelCase word or a lowerCamelCase word.
                        </para>
                        <para>Default is true.</para>
                    </listitem>
                </varlistentry>
    ...
    <doc:template name="str:substring-before-first" xmlns="">
        ...
        <refdescription>
            <para>Extracts the portion of string 'text' which occurs before any of the characters in string 'chars'.
            </para>
        </refdescription>
    ...
    <doc:template name="str:substring-after-last" xmlns="">
        ...
        <refdescription>
            <para>Extracts the portion of string 'text' which occurs after the last of the character in string
                'chars'.
            </para>
        </refdescription>
    ...
    <doc:template name="str:substring-before-last" xmlns="">
        ...
        <refdescription>
            <para>Extracts the portion of string 'text' which occurs before the first character of the last occurance of
                string 'chars'.
            </para>
        </refdescription>
    ...
    <doc:template name="str:count-substring" xmlns="">
        ...
        <refdescription>
            <para>Counts the number of times a substring occurs in a string. This can also counts the number of times a
                character occurs in a string, since a character is simply a string of length 1.
            </para>
        </refdescription>
            ...
            <xsl:when test="contains($text, substring($chars, 1, 1))">
                ...
                <xsl:choose>
                    <xsl:when test="string-length($next-character)">
                        <xsl:variable name="first-character-position"
                                      select="string-length(substring-before($text, substring($chars, 1, 1)))"/>
                        <xsl:variable name="next-character-position"
                                      select="string-length(substring-before($text, $next-character))"/>
...
<xsl:call-template name="str:string-match">
  ...
  <xsl:with-param name="pattern" select="'abc*def?g'"/>
    ...
    <xsl:template name="str:generate-space">
        <xsl:param name="count"/>
        <xsl:choose>
